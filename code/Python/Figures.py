...
#-----------------------------------------------------------------------------#
if EMPRISE.at_MPI:
    res_dir = '/data/pt_02495/emprise7t/derivatives/numprf/'
else:
...
def WP1_Tab2():
    ...
    for gr in sub_group:
        ...
        if '00' in gr[0]:
            ...
            df.to_csv('/data/pt_02495/emprise7t_analysis/Figures/WP1_Table_2_sub-childs'+'_ses-visual.csv', index=False)
        #df.to_excel('Figures_WP1/WP1_Table_2_sub-all_ses-both.xlsx', index=False)

# function: Work Package 1, Table 3
#-----------------------------------------------------------------------------#
def WP1_Tab3(sub_group, ses, model, hemi, space, cv=True, verts=[0,0]):
    ...
    filename = '/data/pt_02495/emprise7t_analysis/Figures/Tables/WP1_Tab3_Rsq_Fwhm'+'_sub-'+sub_group+'_hemi-'+hemi+'.csv'
    ...
    for gr in sub_group:
        for ses in sess:
            ...
            for sub in gr:
                ...
                if len(verts_both) == 2:                                            
                    both = np.concatenate((verts_both[0],verts_both[1]))
                    both_m = np.mean(both)
            ...
            # create data frame
            data = zip(df_subs, df_hemis, df_rsq, df_both, df_max, df_max_mu, df_ind)
            cols = ['sub', 'hemi', 'sep_meanrsq', 'both_meanrsq','maxrsq','maxrsq_mu','maxrsq_ind']
            df    = pd.DataFrame(data, columns=cols)
        
        # save data frame to CSV/XLS
        if '00' in gr[0]:
            df.to_csv('/data/pt_02495/emprise7t_analysis/Figures/WP1_Tab_S1_sub-adults'+'_ses-visual.csv', index=False)
        else:
            df.to_csv('/data/pt_02495/emprise7t_analysis/Figures/WP1_Tab_S1_sub-childs'+'_ses-visual.csv', index=False)
...
def WP1_Fig1(Figure):
    ...
    #sub_visual = '003'
    #sub_audio  = '009'
    subs_ad    = EMPRISE.adults
    subs_ch    = EMPRISE.childs 
    model      = model_def
    space      = 'fsnative'
    filepath   = '/data/pt_02495/emprise7t_analysis/Figures/Fig2/'
    ...
    if Figure == '1':
        ...
        fig = plot_tuning_function_time_course(sub, 'visual', model, 'L', space, verts=[0,0], col = color)
...
def WP1_Fig2(Figure):
 ...
 #------------------------------------------------------------------------------
    ...
    alpha      = 0.05           # p < 0.05
    meth       = 'B'            # Bonferroni
    filepath   = '/data/pt_02495/emprise7t_analysis/Figures/Fig3/'
    ...
    if Figure == '2A':
        ...
        fig.savefig(filepath+'WP1_Figure_2a_sub-'+'113'+'_ses-visual_model-'+model_ab+'.png', dpi=300, transparent=True)
        ...
        fig.savefig(filepath+'WP1_Figure_2b_sub-'+'003'+'_ses-visual_model-'+model_ab+'.png', dpi=300, transparent=True)
    
    # Figure 2B
    if Figure == '2B':
        
        # Figure 2B, Part 1: visual data children
        fig = plot_participant_count(subs_children, 'visual', model, alpha_thr=alpha, meth_str=meth)
        fig.savefig(filepath+'WP1_Figure_2c_sub-children_ses-visual_model-'+model_ab+'.png', dpi=300, transparent=True)
        
        # Figure 2B, Part 2: audio data
        fig = plot_participant_count(subs_adults, 'visual', model, alpha_thr=alpha, meth_str=meth)
        fig.savefig(filepath+'WP1_Figure_2d_sub-adults_ses-visual_model-'+model_ab+'.png', dpi=300, transparent=True)
    ...
    if Figure == '2Bp':
        ...
        fig.savefig(filepath+'WP1_Figure_2c_sub-children_ses-visual_p-'+str(alpha)+'.png', dpi=150, transparent=True)
    ...
    if Figure == '2C':
        ...
        fig.savefig(filepath + 'WP1_Figure_2e_sub-'+'113'+'_ses-visual_model-'+model_ab+'.png', dpi=300, transparent=True)
        
        # Figure 2C, Part 2: visual data adults
        fig = plot_surface_para('003', 'visual', model, space, para='mu', alpha_thr=alpha, meth_str=meth)
        fig.savefig(filepath+'WP1_Figure_2f_sub-'+'003'+'_ses-visual_model-'+model_ab+'.png', dpi=300, transparent=True)
    
    # Figure S3
    if Figure == 'S3':
    
        # Figure S3: all subjects, cvRsq
        for sub in subs_adults:
            for ses in sess:
...
def WP1_Fig4(Figure):
    ...
    # otherwise, create Figure panels
        ...
        mesh       = 'pial'
        AFNI       = True
        filepath   = '/data/pt_02495/emprise7t_analysis/Figures/'
    ...
    if Step == 3:
        ...
        for ses in sess:
            ...
            for k in range(1,len(cons)):
                ...
                filepath = '/data/pt_02495/emprise7t_analysis/Figures/FigS/'
